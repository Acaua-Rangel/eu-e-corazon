export const storySlides = [
  {
    id: "inicio-stories",
    date: "13 de Junho",
    tagline: "O começo",
    title: "O primeiro story",
    image: "/images/20260613.webp",
    description:
      "Tudo começou no dia 13 de junho. A ideia inicial era ir a um bar ver o jogo, mas eu não podia ir e você já tinha marcado de assistir com seus amigos. Então vocês foram assistir no salão do prédio. Quando vi o story de Clara com você, te chamei de linda sem pretensão nenhuma — eu estava até de celibato e achei que Clara só fosse rir e me ignorar. Falei porque você realmente era linda (e continua sendo). Só que a conversa rendeu e nunca mais paramos de nos falar.",
    highlight: "Um elogio sincero e sem pretensão que mudou a minha vida."
  },
  {
    id: "primeiro-dia-casa-dela",
    date: "19 de Junho",
    tagline: "Primeiro dia",
    title: "Assistindo ao jogo",
    image: "/images/IMG-20260619-WA0043.webp",
    description:
      "No dia 19, fui até a sua casa para a gente assistir juntos ao jogo do Brasil contra o Japão. O salão estava fechado, então fomos para a sala de jogos assistir pela tela do celular. O jogo foi super acirrado e a gente dava um beijo a cada gol do Brasil — no final já estávamos quase achando que o Brasil ia perder! Mas foi ali que começamos a ficar de verdade.",
    highlight: "Um beijo a cada gol e o início de nós dois."
  },
  {
    id: "shopping-certeza-1",
    date: "21 de Junho",
    tagline: "Certeza",
    title: "Certeza de nós dois",
    image: "/images/20260621_214435.webp",
    description:
      "Dois dias depois, fomos passear no shopping. Foi naquele dia que eu tive a certeza de que engataríamos um relacionamento. A nossa sintonia, a química, as risadas e até a nossa proximidade me passavam a sensação de que a gente já estava junto há muito tempo. Estar com você parecia tão natural quanto respirar.",
    highlight: "Ali eu soube: não era só um momento, era o começo de nós dois."
  },
  {
    id: "bar-jogo-brasil",
    date: "24 de Junho",
    tagline: "Dates e coincidências",
    title: "O jogo no bar",
    image: "/images/20260624_195547.webp",
    description:
      "Nosso próximo date foi para assistir a mais um jogo do Brasil, dessa vez num bar com a Clara. E por uma coincidência da vida, a Ana (ex da Clara) estava lá no mesmo lugar. A gente tentou dar aquele suporte de amigos, foi uma daquelas situações inusitadas... mas mesmo no meio do caos, ao seu lado tudo se torna divertido e marcante.",
    highlight: "Parceria até nas situações mais inusitadas e inesperadas."
  },
  {
    id: "saga-da-vela-madero",
    date: "29 de Junho",
    tagline: "Saga da vela I",
    title: "Madero",
    video: "/videos/VID-20260629-WA0004.mp4",
    description:
      "A gente combinou um date no Salvador Shopping e a ideia original era ser um date duplo. Só que a outra pessoa não foi... e quem sobrou de vela com a gente foi a Clara. Foi hambúrguer no Madero, muitas risadas da situação e mais um episódio clássico da nossa história.",
    highlight: "Comida boa, risadas sinceras e a Clara sendo nossa fã número 1 de vela."
  },
  {
    id: "saga-da-vela-gattai",
    date: "05 de Julho",
    tagline: "Saga da vela II",
    title: "A vela no Gattai",
    image: "/images/20260705_171334.webp",
    description:
      "No Gattai a história se repetiu e a Clara ficou de vela de novo. O Brasil tinha perdido para a Holanda e ela estava com sentimentos mistos: frustrada por não ter visto Ste, mas aliviada por não tê-la levado para um date onde o Brasil perdeu logo de início. No fim, estar com você transformou qualquer clima em risada.",
    highlight: "Mais um episódio clássico da Clara de vela com a gente."
  },
  {
    id: "museu-arte-bahia-1",
    date: "11 de Julho",
    tagline: "Um dos nossos melhores dias",
    title: "Museus e Sorvete",
    image: "/images/20260711_173317.webp",
    description:
      "Esse dia ficou gravado como um dos melhores encontros da nossa história. Primeiro fomos no Museu de Arte Contemporânea (MAC Bahia) na Graça, admirando a arte e caminhando de mãos dadas. Depois seguimos até o Museu de Arte da Bahia (MAB), no Corredor da Vitória. E lá nos deparamos com a exposição do Mundo Bita.",
    highlight: "'...mas solto e livre no espaço!' — assim como o meu coração com você."
  },
  {
    id: "museu-arte-bahia-2",
    date: "11 de Julho",
    tagline: "Projeções",
    title: "O Mundo Bita",
    image: "/images/20260711_173538.webp",
    description:
      "A exposição interativa com projeção de planetas, astronautas e um coração gigante desenhado na tela foi incrível. Não conseguimos ver o museu todo por completo, mas aquela sala cheia de luzes e cores nos fez voltar a ser crianças, tirando fotos e nos divertindo muito.",
    highlight: "Nenhuma obra de arte de museu chegava perto da sua beleza."
  },
  {
    id: "museu-arte-bahia-3",
    date: "11 de Julho",
    tagline: "Sua espontaneidade",
    title: "O totem do astronauta",
    image: "/images/20260711_174825.webp",
    description:
      "Você colocando o rosto no totem do astronauta do Mundo Bita, no meio do salão do palacete com aquele piso xadrez clássico. É essa sua leveza, seu sorriso solto e sua autenticidade que me conquistam todos os dias.",
    highlight: "Quem tem o privilégio de te ver sorrir tem tudo."
  },
  {
    id: "museu-arte-bahia-4",
    date: "11 de Julho",
    tagline: "Nós dois no Palacete",
    title: "Nós dois no museu",
    image: "/images/IMG-20260711-WA0050.webp",
    description:
      "Nós dois juntos, você segurando meu braço, aquele look impecável com a camisa rosa e você incrível de preto. Aquele dia foi tão bom que eu não queria que terminasse.",
    highlight: "Momentos que a gente guarda para a vida toda."
  },
  {
    id: "tinta-3d-cosplay",
    date: "05 de Agosto",
    tagline: "O projeto do Bon Odori",
    title: "O traje do Homem-Aranha",
    image: "/images/IMG-20260805-WA0026.webp",
    description:
      "A gente se planejou com antecedência para ir ao Bon Odori de cosplay combinando. Eu comprei o traje do Homem-Aranha indiano (Pavitr Prabhakar) e, como eu gosto de caprichar nos detalhes, decidimos personalizar tudo com tinta tridimensional Acrilex 3D em alto relevo.",
    highlight: "Dedicação aos detalhes em tudo que a gente constrói junto."
  },
  {
    id: "cosplay-finalizado",
    date: "06 de Agosto",
    tagline: "A aranha",
    title: "A aranha no peito",
    image: "/images/IMG-20260806-WA0009.webp",
    description:
      "Fui na sua casa para pintar a aranha no peito. Na hora quase não deu certo pela pressa, mas terminei em casa com calma e o resultado ficou excelente.",
    highlight: "Trabalho em equipe que dá certo."
  },
  {
    id: "sorveteria-ribeira",
    date: "06 de Agosto",
    tagline: "Pausa durante a pintura",
    title: "Pausa para o sorvete",
    image: "/images/20260806_163229.webp",
    description:
      "No mesmo dia em que estávamos fazendo a pintura da aranha no peito do cosplay na sua casa, decidimos fazer uma pausa merecida para tomar um sorvete na Sorveteria da Ribeira, que fica bem perto. Olhar para você saboreando o sorvete com calma é uma daquelas memórias que guardo com muito carinho.",
    highlight: "Uma pausa boa no meio da nossa produção."
  },
  {
    id: "cinema-aranhaverso-1",
    date: "08 de Agosto",
    tagline: "No cinema",
    title: "Cinema no colo",
    image: "/images/20260808_150214.webp",
    description:
      "Fomos ao cinema com João e a amiga dele para ver o novo filme do Homem-Aranha. Foi uma correria para aprontar o traje, mas fomos com o que já estava feito — e você também estava com o seu cosplay combinando. Mesmo que as pessoas ali não soubessem que estávamos de cosplay, a gente sabia. Te carregar no colo em frente ao banner foi o ponto alto.",
    highlight: "Mesmo que ninguém soubesse do cosplay, a gente sabia."
  },
  {
    id: "cinema-aranhaverso-2",
    date: "08 de Agosto",
    tagline: "Sessão 3D",
    title: "Sessão 3D",
    image: "/images/IMG-20260808-WA0036.webp",
    description:
      "Nós dois na sala de cinema com os óculos 3D e pipoca, prontos para a sessão. Compartilhar essa loucura boa e esse momento com você ao meu lado não tem preço.",
    highlight: "A melhor companhia para todas as aventuras."
  },
  {
    id: "pituacu-paisagem",
    date: "15 de Agosto",
    tagline: "Parque de Pituaçu",
    title: "A trilha em Pituaçu",
    image: "/images/IMG-20260815-WA0016.webp",
    description:
      "Tiramos o dia para passear no Parque Metropolitano de Pituaçu. O dia estava limpo, a lagoa refletindo o sol e resolvemos entrar na trilha... só não imaginávamos que o percurso era tão longo. Andamos bastante até perceber que não daria tempo de completar a volta toda.",
    highlight: "A trilha sem fim que virou uma das nossas melhores histórias."
  },
  {
    id: "pituacu-degraus",
    date: "15 de Agosto",
    tagline: "Só nós dois",
    title: "Tempo só nosso",
    image: "/images/IMG-20260815-WA0013.webp",
    description:
      "Mesmo tendo retornado no meio do caminho, foi um momento muito bom. Um tempo só nosso, longe da rotina, conversando e aproveitando a companhia um do outro na sombra das árvores.",
    highlight: "Não importa o destino da caminhada, desde que eu esteja caminhando com você."
  },
  {
    id: "pituacu-banco-coracoes",
    date: "15 de Agosto",
    tagline: "Fim de tarde na grama",
    title: "O banco na grama",
    image: "/images/IMG-20260815-WA0021.webp",
    description:
      "Descansando no banco sobre a grama, sob a luz do fim de tarde, abraçados. A gente não precisava de mais nada para aproveitar o momento.",
    highlight: "A paz de ter você ao meu lado."
  },
  {
    id: "pizzaria-sushi",
    date: "15 de Agosto",
    tagline: "Para fechar o dia",
    title: "A pizza no fim do dia",
    image: "/images/IMG-20260815-WA0019.webp",
    description:
      "Depois de toda a caminhada, fomos naquela pizzaria que também servia sushi. O atendimento no início deixou a desejar... mas quando a pizza chegou, estava excelente. Comemos, rimos da situação e fechamos o dia muito bem.",
    highlight: "Com você, até os imprevistos viram boas histórias."
  }
];



